...
Los mercados cambian de régimen. Medias, volatilidades y correlaciones estimadas en entrenamiento pueden no persistir. El experimento original utiliza un único corte 80/20; la extensión \textit{walk-forward} aporta nueve ventanas, pero sus resultados siguen siendo limitados por el universo, el periodo y las decisiones fijas del diseño. Por ello las métricas no describen estabilidad general a lo largo de múltiples mercados y configuraciones.

\subsection{Modelos generativos}

El VAE principal representa una configuración concreta. La comparación AE/VAE se repitió con tres semillas y una variante adicional, y la extensión temporal reentrenó el VAE en nueve ventanas, pero la evidencia sigue siendo limitada: dos dimensiones latentes, un valor de $\beta$ y un único universo. El VAE suaviza volatilidades y altera correlaciones en las muestras. El análisis adicional de colas tampoco cubrió los peores retornos diarios observados en 2018 y COVID-19.

GAN, WGAN y la arquitectura híbrida considerada inicialmente no fueron implementadas. Se mantienen en teoría y prospectiva. Esta diferencia entre diseño inicial y ejecución debe conservarse explícita. Tampoco se evaluaron difusión, flujos normalizantes o modelos temporales condicionados.
...
El tipo libre de riesgo se fija en cero. La evaluación incorpora CVaR 95~\% como extensión, pero no VaR completo por estrategia, Sortino ni análisis de concentración completo. La cartera VAE con mayor retorno también tiene mayor volatilidad; una función de utilidad diferente podría seleccionar otros pesos.
