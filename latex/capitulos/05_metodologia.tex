...
En primer lugar, se adquirieron datos históricos de activos del S\&P 500 mediante la herramienta \textit{yfinance}. Con Python se preprocesaron los datos para alimentar el entrenamiento del modelo VAE, con el objetivo de extraer una representación latente subyacente a la distribución de los retornos observados.
...
Mediante la herramienta \textit{yfinance}, se creó un pipeline para adquirir series históricas de diez años de una selección de activos del S\&P 500.
...
\item Análisis complementario de la calidad distribucional mediante medidas informacionales, como la divergencia de Kullback-Leibler, sin utilizarla como función objetivo directa de la cartera.
\end{itemize}
...
\begin{itemize}
\item Retorno acumulado: se utilizó para evaluar el rendimiento total durante el periodo de evaluación.
\item Retorno anualizado: permitió comparar rentabilidades en términos homogéneos.
\item Volatilidad anualizada: sirvió como medida de riesgo, evaluando la variabilidad de los retornos.
\item Ratio de Sharpe: midió la rentabilidad ajustada por riesgo.
\item Máximo drawdown: midió la mayor caída desde un máximo hasta un mínimo posterior, evaluando el riesgo extremo.
\item Divergencia de Kullback-Leibler: se empleó como medida complementaria para evaluar la distancia entre la distribución de retornos generada por el modelo y la distribución observada, no como función objetivo financiera.
\end{itemize}
