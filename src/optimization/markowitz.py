...
WEIGHTS_OUTPUT_FILE = GENERATED_DIR / "markowitz_weights.csv"
RETURNS_OUTPUT_FILE = GENERATED_DIR / "markowitz_returns.csv"
METRICS_OUTPUT_FILE = GENERATED_DIR / "markowitz_metrics.csv"
...
def negative_sharpe_ratio(
    ...
    Se minimiza el negativo para obtener la cartera con mayor Sharpe.
    """
...
def optimize_markowitz_portfolio(train_returns: pd.DataFrame) -> pd.DataFrame:
    """
    Optimiza una cartera long-only mediante el modelo clásico media-varianza.

    Restricciones:
    - La suma de pesos debe ser igual a 1.
    - No se permiten posiciones cortas: cada peso debe estar entre 0 y 1.
    """
    ...
    if not result.success:
        raise RuntimeError(f"La optimización de Markowitz no ha convergido: {result.message}")
...
def compute_portfolio_test_returns(
    ...
    portfolio_returns.name = "markowitz_return"

    return portfolio_returns


def run_markowitz_benchmark() -> None:
    """
    Ejecuta el benchmark completo de Markowitz:
    1. Carga retornos train/test.
    2. Optimiza pesos con train.
    3. Evalúa la cartera en test.
    4. Guarda pesos, retornos y métricas.
    """
    GENERATED_DIR.mkdir(parents=True, exist_ok=True)

    train_returns, test_returns = load_returns()

    weights_df = optimize_markowitz_portfolio(train_returns)
    ...
    print("Benchmark de Markowitz completado.")
